...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($entity_types as $type => $info) {
      if (is_a($info, 'Drupal\Core\Entity\ContentEntityType')) {
        ...
        if (!in_array($type, array('block_content', 'contact_message', 'file', 'menu_link_content', 'node', 'shortcut'))) {
        ...
        if (!empty($settings['forward_entity_' . $type]) || ($type == 'node')) {
          ...
          if (count($bundles) == 1) {
            ...
            $form['forward_entities']['forward_' . $type . '_types'] = array(
              ...
              '#options' => $options,
              '#default_value' => $defaults,
              '#description' => $this->t('Choose @type bundles to show Forward on.', array('@type' => $info->getLowercaseLabel())),
            );
          }
        }
      }
    ...
    // View Modes.
    $modes = array('full' => t('Full entity'), 'teaser' => t('Teaser'));
    $options = array();
    $defaults = array();
    ...
    $form['forward_interface']['forward_interface_type'] = array(
      ...
      '#options' => array('link' => t('Link to separate page'), 'form' => t('Display inline form')),
    ...
    $form['forward_interface']['forward_link_options'] = array(
      ...
      '#states' => array(
        'visible' => array(
          ':input[name=forward_interface_type]' => array('value' => 'link'),
        ),
      ),
    );
    ...
    $form['forward_interface']['forward_link_options']['forward_link_style'] = array(
      ...
      '#options' => array(0 => $this->t('Text only'), 1 => t('Icon only'), 2 => t('Icon and text')),
    ...
    $form['forward_form']['personal_messages']['forward_personal_message'] = array(
      ...
      '#options' => array(0 => 'Hidden', 1 => 'Optional', 2 => 'Required'),
    ...
    $form['forward_form']['personal_messages']['forward_personal_message_filter'] = array(
      ...
      '#states' => array(
        'invisible' => array(
          ':input[name=forward_personal_message]' => array('value' => 0),
        ),
      ),
    );
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('forward_link_icon')) {
      ...
      if (count($errors)) {
        $form_state->setErrorByName('forward_link_icon', $this->t("The link icon path '@path' is invalid.", array('@path' => $form_state->getValue('forward_link_icon'))));
      }
    }
    ...
    foreach ($entity_types as $type => $info) {
      if (is_a($info, 'Drupal\Core\Entity\ContentEntityType')) {
        if (!empty($values[$type])) {
          ...
          $this->config('forward.settings')->set('forward_entity_' . $type, FALSE);
        }
        $bundles = $this->bundleInfoManager->getBundleInfo($type);
        foreach ($bundles as $bundle => $bundle_info) {
          ...
          if (!empty($values[$type]) && !empty($bundle_values[$bundle])) {
            ...
            $this->config('forward.settings')->set('forward_' . $type . '_' . $bundle, FALSE);
          }
          // If only one bundle, it gets the same setting as its type.
          if (count($bundles) == 1) {
            $this->config('forward.settings')->set('forward_' . $type . '_' . $bundle, $this->config('forward.settings')->get('forward_entity_' . $type));
          }
        }
      }
    }
